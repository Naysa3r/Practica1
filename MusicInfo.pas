...
procedure TForm1.miDelAlbumClick(Sender: TObject);
var
  SCode: string;
  InputCode: Integer;

  CurrAlb, PrevAlb: PAlbum;
  AlbFound: Boolean;

  CurrSong, PrevSong, TempSong: PSong;
begin
  SCode := InputBox('Удаление альбома', 'Введите код альбома для удаления:', '');
  if Trim(SCode) = '' then Exit;
  InputCode := StrToIntDef(SCode, -1);

  // Удаление всех песен, привязанных к альбому
  CurrSong := HeadSongs;
  PrevSong := nil;
  while CurrSong <> nil do
  begin
    if CurrSong^.AlbumCode = InputCode then
    begin
      TempSong := CurrSong;
  ...
  begin
    if CurrAlb^.AlbumCode = InputCode then
    begin
      AlbFound := True;
      if PrevAlb = nil then
  ...
  if AlbFound then
    ShowMessage('Альбом с кодом ' + SCode + ' и все его песни успешно удалены из ОЗУ.')
  else
    ShowMessage('Альбом с таким кодом не найден.');
end;
